...
def fresnel_equation(alpha_i: float = 0, n_rel: float = 1) -> float:
    ...
    rs = np.abs(
        (np.cos(alpha_i) - n_rel * cos_alpha_t) / (np.cos(alpha_i) + n_rel * cos_alpha_t)
    ) ** 2
    rp = np.abs(
        (cos_alpha_t - n_rel * np.cos(alpha_i)) / (cos_alpha_t + n_rel * np.cos(alpha_i))
    ) ** 2
    specular_reflection = 0.5 * (rs + rp)
    return specular_reflection


def cylindrical_distance(rtz: Tuple[float, float, float], rtz_prime: Tuple[float, float, float]) -> float:
    r, theta, z = rtz
...
def diffusion_approximation(
    ...
    Rd = (a_prime * z_prime * (1 + mu_eff * rho1) * np.exp(-mu_eff * rho1) / (4 * np.pi * rho1 ** 3)) + (
            a_prime * (z_prime + 4 * D) * (1 + mu_eff * rho2) * np.exp(-mu_eff * rho2) / (
            4 * np.pi * rho2 ** 3))
    Rd *= (1 - Rsp)
    return Rd
...
def create_diffusion_approximation(
    ...
    return functools.partial(diffusion_approximation, r=r, alpha_i=alpha_i, n_rel=n_rel, g=g)
